...
<p align="center"><sub>他の言語で読む: [🇬🇧 English](README.md) · [🇪🇸 Español](README.es.md) · [🇧🇷 Português (BR)](README.pt-BR.md) · [🇨🇳 中文](README.zh-CN.md) · **🇯🇵 日本語**</sub></p>
...
| 項目 | 数 |
|---|---|
| アクティブソース (ROSTER + BENCH) | **20** |
| 収集済みアイテム総数 | **28,417** |
| クロスソース信号ツール (x≥2) | **1,479** |
| ⭐ GEM (業界標準) | **200** |
| 👍 試す価値あり | **335** |
...
1. **Discovery**: GitHub API (GraphQL バッチ) でアクティブに維持されている `awesome-*` リポを検出。
2. **多次元スコアリング**: freshness、アクティビティ、人気、深度、コミュニティヘルスを重み付けして単一スコアに。
3. **クロスソース重複排除**: 2 つ以上の awesome に現れるアイテムを単一の `CuratedTool` にグループ化。`appearsInCount` は合意シグナル。
4. **AI 分類**: Claude (Anthropic) が `GEM | WORTH_TRYING | MEH | HYPE | DEAD` をアイテムごとに推論付きで割り当て。
5. **人間の判定**: 人間のキュレーターが AI の提案を確認または上書き。👤 マークは人間検証済み。

技術詳細 → [juanchi.dev ブログシリーズ](https://juanchi.dev/blog/series/awesome-curated)
...
### AI Framework
- **[Vercel AI](https://github.com/vercel/ai)** — - Vercel AI is a TypeScript toolkit designed to help you build AI-powered applications using popular frameworks like Next.js, React, Svelte, Vue and runtimes like Node.js. — 3 個のリストに登場 · `typescript` `nextjs` `react` `llm-integration`
...
### C++
- **[TensorFlow](https://github.com/tensorflow/tensorflow)** — - TensorFlow is a leading library designed for developing and deploying state-of-the-art machine learning applications. — 🔥 6 個のリストに登場 · `machine-learning` `deep-learning` `python` `c++`
- **[LightGBM](https://github.com/microsoft/lightgbm)** — Microsoft's fast, distributed, high performance gradient boosting (GBDT, GBRT, GBM or MART) framework based on decision tree algorithms, used for ranking, classification and many other machine… — 3 個のリストに登場 · `machine-learning` `gradient-boosting` `distributed` `python`
...
### Data Visualization
- **[matplotlib](https://github.com/matplotlib/matplotlib)** — - A Python 2D plotting library which produces publication-quality figures in a variety of hardcopy formats and interactive environments across platforms. — 3 個のリストに登場 · `feature-engineering` `python` `sklearn` `ml`
- **[Streamlit](https://github.com/streamlit/streamlit)** — - Streamlit lets you create apps for your machine learning projects with deceptively simple Python scripts. It supports hot-reloading, so your app updates live as you edit and save your file. — 3 個のリストに登場 · `python` `web-framework` `ml-apps` `rapid-prototyping`
...
### Deep Learning
- **[Stable Baselines](https://github.com/dlr-rm/stable-baselines3)** — - A fork of OpenAI Baselines, implementations of reinforcement learning algorithms. — 3 個のリストに登場 · `reinforcement-learning` `python` `pytorch` `tensorflow`
...
### Developer Tools
- **[Docker](https://docker.com)** — Powerful, performs operating-system-level virtualization. [![Open-Source Software][OSS Icon]](https://github.com/docker) ![Freeware][Freeware Icon] [![Awesome List][awesome-list… — 4 個のリストに登場 · `containers` `devops` `virtualization`
- **[mitmproxy](https://mitmproxy.org)** — A Python tool used for intercepting, viewing and modifying network traffic. Invaluable in troubleshooting certain problems. — 3 個のリストに登場 · `python` `proxy` `debugging` `security`
- **[Wireshark](https://wireshark.org)** — The world’s foremost and widely-used network protocol analyzer. [![Open-Source Software][OSS Icon]](https://github.com/wireshark/wireshark) ![Freeware][Freeware Icon] — 3 個のリストに登場 · `network` `pcap` `wireshark` `cli-gui`

### Development Tools
- **[Visual Studio Code](https://code.visualstudio.com)** — Microsoft's free & open-source editor, TypeScript friendly, [VSCode Plugins](editor-plugin.md#vscode-plugin). [![Open-Source Software][OSS Icon]](https://github.com/Microsoft/vscode)… — 4 個のリストに登場 · `editor` `typescript` `vscode-plugins`

### Disk Analysis
- **[dust](https://github.com/bootandy/dust)** — A more intuitive version of `du` in Rust. · `rust` `cli` `disk-analysis` `unix-tool`

### Distributed Computing
- **[RLlib](https://github.com/ray-project/ray)** — RLlib is an industry level, highly scalable RL library for tf and torch, based on Ray. It's used by companies like Amazon and Microsoft to solve real-world decision making problems at scale. — 3 個のリストに登場 · `reinforcement-learning` `ray` `python` `distributed`
...
### Distributed Systems
- **[ZooKeeper](https://zookeeper.apache.org)** — ZooKeeper is a centralized service for maintaining configuration information, naming, providing distributed synchronization, and providing group services. — 3 個のリストに登場 · `java` `distributed-coordination` `apache` `consensus`
...
### Go
- **[Kubernetes](https://github.com/kubernetes/kubernetes)** — _(label: good first issue)_ <br> Production-Grade Container Scheduling and Management System — 3 個のリストに登場 · `kubernetes` `container-orchestration` `go` `distributed-systems`
- **[Moby](https://github.com/moby/moby)** — Collaborative project for the container ecosystem to assemble container-based systems. — 3 個のリストに登場 · `go` `containers` `docker` `open-source`
...
### JavaScript
- **[Jest](https://github.com/facebook/jest)** — _(label: good first issue)_ <br> A complete and easy to set up JavaScript testing solution. — 🔥 5 個のリストに登場 · `testing` `javascript` `typescript` `mocking`
- **[Next.js](https://github.com/vercel/next.js)** — _(label: good first issue)_ <br> A minimalistic framework for universal server-rendered React applications — 🔥 5 個のリストに登場 · `react` `ssr` `typescript` `full-stack`
- **[Netron](https://github.com/lutzroeder/netron)** — - Netron is a viewer for neural network, deep learning and machine learning models. — 4 個のリストに登場 · `neural-networks` `visualization` `ml-models`
- **[Vite](https://github.com/vitejs/vite)** — _(label: good first issue)_ <br> Next generation frontend tooling. It's fast! Alternative to Create React App — 4 個のリストに登場 · `build-tool` `frontend` `javascript` `typescript`
- **[Babel](https://github.com/babel/babel)** — _(label: good first issue)_ <br> A compiler for writing next generation JavaScript. — 3 個のリストに登場 · `javascript` `transpiler` `babel` `ast`
- **[D3js](https://d3js.org)** — is a powerful data visualization javascript library. — 3 個のリストに登場 · `javascript` `visualization` `svg` `data-driven`
- **[ESLint](https://github.com/eslint/eslint)** — _(label: good first issue)_ <br> A fully pluggable tool for identifying and reporting on patterns in JavaScript. — 3 個のリストに登場 · `javascript` `linting` `eslint` `typescript`
- **[React Native](https://github.com/facebook/react-native)** — _(label: Good-first-issue)_ <br> A framework for building native apps with React. — 3 個のリストに登場 · `javascript` `react` `mobile` `cross-platform`
- **[Vue.js](https://github.com/vuejs/vue)** — _(label: good first issue)_ <br> The Progressive JavaScript Framework. — 3 個のリストに登場 · `javascript` `vue` `spa` `frontend`

### JavaScript Runtime
- **[Node.js](https://github.com/sindresorhus/awesome-nodejs)** — Async non-blocking event-driven JavaScript runtime built on Chrome's V8 JavaScript engine. — 4 個のリストに登場 · `nodejs` `javascript` `runtime`
...
- **[Obsidian](https://obsidian.md)** — Obsidian is a powerful knowledge base on top of a local folder of plain text Markdown files. — 4 個のリストに登場 · `markdown` `local-first` `knowledge-base` `self-hosted`
...
### Machine Learning
- **[BayesWitnesses/m2cgen](https://github.com/bayeswitnesses/m2cgen)** — A CLI tool to transpile trained classic machine learning models into a native Rust code with zero dependencies. [](https://github.com/BayesWitnesses/m2cgen/actions) — 🔥 7 個のリストに登場 · `machine-learning` `code-generation` `model-export` `open-source`
- **[XGBoost](https://github.com/dmlc/xgboost)** — - XGBoost is an optimized distributed gradient boosting library designed to be highly efficient, flexible and portable. — 🔥 5 個のリストに登場 · `machine-learning` `gradient-boosting` `python` `distributed`
- **[Apache Spark](https://github.com/apache/spark)** — - Micro-batch processing for streams using the apache spark framework as a backend supporting stateful exactly-once semantics. — 3 個のリストに登場 · `ai-agents` `memory` `python` `storage`
...
- **[SpaCy](https://github.com/explosion/spacy)** — - spaCy is a library for advanced Natural Language Processing in Python and Cython. — 3 個のリストに登場 · `nlp` `python` `spacy` `nlp-pipeline`
...
### Python
- **[Pytorch](https://github.com/pytorch/pytorch)** — _(label: good first issue)_ <br> PyTorch is an open source machine learning library based on the Torch library, used for applications such as computer vision and natural language processing. — 🔥 6 個のリストに登場 · `deep-learning` `machine-learning` `python` `gpu`
- **[Jupyter Notebooks](https://github.com/jupyter/notebook)** — - Web interface python sandbox environments for reproducible development — 3 個のリストに登場 · `python` `jupyter` `notebook` `interactive`
- **[PyG](https://github.com/pyg-team/pytorch_geometric)** — - PyG (PyTorch Geometric) is a library built upon PyTorch to easily write and train Graph Neural Networks (GNNs) for a wide range of applications related to structured data. — 3 個のリストに登場 · `gnn` `pytorch` `python` `ml`
- **[scikit-learn](https://github.com/scikit-learn/scikit-learn)** — - Scikit-learn is a powerful machine learning library that provides a wide variety of modules for data access, data preparation and statistical model building. — 3 個のリストに登場 · `python` `machine-learning` `sklearn` `supervised`
- **[SymPy](https://github.com/sympy/sympy)** — _(label: Easy-to-Fix)_ <br> A Python library for symbolic mathematics. — 3 個のリストに登場 · `python` `mathematics` `symbolic-computation` `open-source`
- **[Transformers](https://github.com/huggingface/transformers)** — - Huggingface's library of state-of-the-art pretrained models for Natural Language Processing (NLP). — 3 個のリストに登場 · `python` `nlp` `transformers` `pytorch`
...
### Testing E2E
- **[Playwright](https://github.com/microsoft/playwright)** — Node.js library to automate Chromium, Firefox and WebKit with a single API. — 4 個のリストに登場 · `testing` `e2e` `automation` `browser`
...
### Tools
- **[DVC](https://github.com/iterative/dvc)** — Data Science Version Control is an open-source version control system for machine learning projects with pipelines support. It makes ML projects reproducible and shareable. — 3 個のリストに登場 · `data-science` `version-control` `ml-pipelines`
...
### TypeScript
- **[Visual Studio Code](https://github.com/microsoft/vscode)** — New type of tool that combines the simplicity of a code editor with what developers need for their core edit-build-debug cycle. Code provides comprehensive editing and debugging support, an… — 4 個のリストに登場 · `editor` `typescript` `open-source` `debugging`

### Utilities
- **[Bitwarden](https://bitwarden.com)** — Open source password management tool for Mac OS, iOS and browsers. [![Open-Source Software][OSS Icon]](https://github.com/bitwarden) ![Freeware][Freeware Icon] — 3 個のリストに登場 · `password-manager` `security` `open-source` `self-hosted`
...
### Web Servers
- **[grpc](https://github.com/grpc/grpc)** — Remote Procedure Calls (RPCs) provide a useful abstraction for building distributed applications and services. The libraries in this repository provide a concrete implementation of the gRPC… — 3 個のリストに登場 · `rpc` `http2` `distributed-systems` `protobuf`
...
### AI Memory Layer
- **[Mem0](https://github.com/mem0ai/mem0)** — - Mem0 enhances AI assistants and agents with an intelligent memory layer, enabling personalized AI interactions. — 3 個のリストに登場 · `ai-memory` `python` `agents` `llm`
...
### Applications
- **[Sniffnet](https://github.com/gyulyvgc/sniffnet)** — Cross-platform application to monitor your network traffic with ease [](https://github.com/GyulyVGC/sniffnet/blob/main/.github/workflows/rust.yml) [](https://crates.io/crates/sniffnet) — 4 個のリストに登場 · `rust` `networking` `open-source` `packet-sniffer`
...
### Browsers
- **[Vivaldi](https://vivaldi.com)** — The browser that puts you in control. ![Freeware][Freeware Icon] — 3 個のリストに登場 · `browser` `chromium` `customization`
...
### C#
- **[Cake](https://github.com/cake-build/cake)** — _(label: Good-first-issue)_ <br> Cake (C# Make) is a free and open source cross-platform build automation system with a C# DSL for tasks such as compiling code, copying files and folders, running… — 3 個のリストに登場 · `csharp` `build-automation` `dsl` `ci-cd`

### C++
- **[Polyaxon](https://github.com/polyaxon/polyaxon)** — - A platform for reproducible and scalable machine learning and deep learning on kubernetes - [(Video)](https://www.youtube.com/watch?v=Iexwrka_hys). — 3 個のリストに登場 · `kubernetes` `ml-ops` `reproducibility` `python`
...
### CLI Tools
- **[yt-dlp](https://github.com/yt-dlp/yt-dlp)** — Downloads videos from almost any online platform, along with information, thumbnails, subtitles, descriptions, and comments (comments only on a select few sites like Youtube and a few small… — 3 個のリストに登場 · `cli` `video-download` `python` `metadata-extraction`
...
- **[Bencher](https://bencher.dev)** — Suite of continuous benchmarking tools designed to catch performance regressions in CI. — 4 個のリストに登場 · `benchmarking` `ci-cd` `rust`
...
### Data Visualization
- **[gradio](https://github.com/gradio-app/gradio)** — - Quickly create and share demos of models - by only writing Python. Debug models interactively in your browser, get feedback from collaborators, and generate public links without deploying anything. — 3 個のリストに登場 · `python` `web-ui` `demos` `visualization`
...
### Developer Tools
- **[Rider](https://jetbrains.com/rider)** — A cross-platform C# IDE based on the IntelliJ platform and ReSharper — 3 個のリストに登場 · `csharp` `ide` `dotnet` `jetbrains`

### Developer Utilities
- **[intelli-shell](https://github.com/lasantosr/intelli-shell)** — Bookmark commands with placeholders and search or autocomplete at any time [](https://crates.io/crates/intelli-shell) [](https://github.com/lasantosr/intelli-shell/actions/workflows/release.yml) — 3 個のリストに登場 · `rust` `cli` `productivity` `shell`
...
### Distributed Systems
- **[Dragonfly](https://github.com/dragonflyoss/dragonfly2)** — Provide efficient, stable and secure file distribution and image acceleration based on p2p technology to be the best practice and standard solution in cloud native architectures. — 3 個のリストに登場 · `p2p` `oci-images` `container-registry` `distributed-systems`
...
### Editor Plugins
- **[Cline](https://marketplace.visualstudio.com/items)** — Autonomous coding agent for VS Code that can create/edit files, execute commands, and use the browser with user permission. Supports multiple AI providers including OpenRouter, Anthropic, OpenAI,… — 🔥 7 個のリストに登場 · `vscode-extension` `ai-agent` `autonomous-coding` `multi-provider`
...
### Go Tools
- **[OctoLinker](https://github.com/octolinker/browser-extension)** — Navigate through `projects.json` files efficiently with the OctoLinker browser extension for GitHub. — 3 個のリストに登場 · `github` `browser-extension` `typescript` `navigation`
...
### Image Processing
- **[Scikit-Image](https://github.com/scikit-image/scikit-image)** — A collection of algorithms for image processing in Python. — 3 個のリストに登場 · `python` `image-processing` `computer-vision` `scipy`
...
### JavaScript
- **[Gatsby.js](https://github.com/gatsbyjs/gatsby)** — _(label: good first issue)_ <br> Build blazing fast, modern apps and websites with React. — 4 個のリストに登場 · `react` `static-site-generator` `graphql`
- **[Ghost](https://github.com/tryghost/ghost)** — _(label: good first issue)_ <br> Just a blogging platform — 3 個のリストに登場 · `javascript` `node.js` `cms` `self-hosted`
- **[Vest](https://github.com/ealush/vest)** — _(label: good first issue)_ <br> Validations framework inspired by unit testing frameworks. — 3 個のリストに登場 · `javascript` `validation` `forms` `typescript`
- **[webdriver.io](https://github.com/webdriverio/webdriverio)** — _(label: first-timers-only)_ <br> Next-gen browser and mobile automation test framework for Node.js — 3 個のリストに登場 · `javascript` `e2e-testing` `webdriver` `automation`
...
### Learning Resource
- **[CTFs](https://github.com/apsdehal/awesome-ctf)** — Capture The Flag frameworks, libraries, etc. — 3 個のリストに登場 · `ctf` `security` `awesome-list`
...
### Machine Learning
- **[CatBoost](https://github.com/catboost/catboost)** — General purpose gradient boosting on decision trees library with categorical features support out of the box. It is easy to install, contains fast inference implementation and supports CPU and GPU… — 4 個のリストに登場 · `gradient-boosting` `machine-learning` `categorical-features` `gpu`
- **[Feature Engine](https://github.com/feature-engine/feature_engine)** — - Feature-engine is a Python library that contains several transformers to engineer features for use in machine learning models. — 3 個のリストに登場 · `spark` `streaming` `scala` `java`
- **[H2O-3](https://github.com/h2oai/h2o-3)** — - Fast scalable Machine Learning platform for smarter applications: Deep Learning, Gradient Boosting & XGBoost, Random Forest, Generalized Linear Modeling (Logistic Regression, Elastic Net),… — 3 個のリストに登場 · `machine-learning` `automl` `java` `distributed`
...
### Misc
- **[awesome](https://github.com/sindresorhus/awesome)** — zap: - A curated list of awesome lists. — 4 個のリストに登場 · `curated-list` `discovery` `meta-index`
...
### Performance Testing
- **[bencher](https://github.com/bencherdev/bencher)** — A suite of continuous benchmarking tools designed to catch performance regressions in CI. — 3 個のリストに登場 · `benchmarking` `ci-cd` `performance` `monitoring`
...
### Python
- **[Bokeh](https://github.com/bokeh/bokeh)** — - Bokeh is an interactive visualization library for Python that enables beautiful and meaningful visual presentation of data in modern web browsers. — 4 個のリストに登場 · `python` `data-visualization` `interactive-charts` `open-source`
- **[Colossal-AI](https://github.com/hpcaitech/colossalai)** — - A unified deep learning system for big model era, which helps users to efficiently and quickly deploy large AI model training and inference. — 3 個のリストに登場 · `python` `llm` `distributed-training` `gpu`
- **[Deepchecks](https://github.com/deepchecks/deepchecks)** — - Deepchecks is a holistic open-source solution for all of your AI & ML validation needs, enabling you to test your data and models from research to production thoroughly. — 3 個のリストに登場 · `python` `ml-validation` `testing` `data-quality`
- **[PyMC](https://github.com/pymc-devs/pymc)** — _(label: beginner friendly)_ <br> A Python library for Bayesian statistical modeling and probabilistic machine learning. Beginner-friendly with 'good first issue' labels. — 3 個のリストに登場 · `bayesian` `probabilistic-ml` `statistics` `open-source`
...
### Resources
- **[Docker for novices](https://youtube.com/watch)** — An introduction to Docker for developers and testers who have never used it. (Video 1h40, recorded linux.conf.au 2019 — Christchurch, New Zealand) by Alex Clews. — 🔥 16 個のリストに登場 · `docker` `containers` `beginners` `video`
...
### Rust
- **[Readest](https://github.com/readest/readest)** — _(label: good first issue)_ <br> A modern, feature-rich ebook reader designed for avid readers offering seamless cross-platform access, powerful tools, and an intuitive interface. — 3 個のリストに登場 · `rust` `desktop-app` `ebook-reader` `cross-platform`
...
### Security
- **[themis](https://github.com/cossacklabs/themis)** — High level crypto library for storing data (AES), secure messaging (ECC + ECDSA / RSA + PSS + PKCS#7) and session-oriented, forward secrecy data exchange (ECDH key agreement, ECC & AES… — 🔥 7 個のリストに登場 · `cryptography` `security` `multi-platform` `open-source`
...
### Security Defense
- **[Honeypots](https://github.com/paralax/awesome-honeypots)** — Deception trap, designed to entice an attacker into attempting to compromise the information systems in an organization. — 3 個のリストに登場 · `security` `honeypots` `threat-detection` `curated-list`
...
- **[juspay/neurolink](https://github.com/juspay/neurolink)** — 📇 ☁️ 🏠 🍎 🪟 🐧 - Making enterprise AI infrastructure universally accessible. Edge-first platform unifying 12 providers and 100+ models with multi-agent orchestration, HITL workflows, guardrails… — 3 個のリストに登場 · `edge-computing` `multi-agent` `enterprise` `orchestration`
...
### Software Packages
- **[lstags](https://github.com/ivanilves/lstags)** — Tool and API to sync Docker images across different registries. — 3 個のリストに登場 · `docker` `registry` `devops` `golang`
...
### Swift
- **[Awesome Core ML Models](https://github.com/likedan/awesome-coreml-models)** — A curated list of machine learning models in CoreML format. — 3 個のリストに登場 · `swift` `coreml` `ios` `curated-list`
...
### Terminal Apps
- **[Wave](https://github.com/wavetermdev/waveterm)** — Open-source terminal with built-in file previews, web browsing, and AI tools. [![Open-Source Software][OSS Icon]](https://github.com/wavetermdev/waveterm) ![Freeware][Freeware Icon] — 3 個のリストに登場 · `terminal` `rust` `ai-integration` `open-source`
...
### Test Framework
- **[Mocha](https://github.com/mochajs/mocha)** — _(label: good first issue)_ <br> Javascript test framework for Node.js and the browser. — 3 個のリストに登場 · `javascript` `testing` `nodejs` `browser`
...
### TypeScript
- **[Amplication](https://github.com/amplication/amplication)** — _(label: good first issue)_ <br> Amplication is an open-source development tool. It helps you develop quality Node.js applications without spending time on repetitive coding tasks. — 3 個のリストに登場 · `code-generation` `nodejs` `backend` `low-code`
- **[Vitest](https://github.com/vitest-dev/vitest)** — _(label: good first issue)_ <br> A blazing fast unit test framework powered by Vite. — 3 個のリストに登場 · `typescript` `testing` `vite` `jest-alternative`
...
### Videos
- **[Functional Programming with TypeScript](https://youtube.com/playlist)** — Discover functional programming with Typescript and create a library like fp-ts alongside Sahand Javid in this beginner-friendly YouTube playlist. — 🔥 11 個のリストに登場 · `typescript` `functional-programming` `fp-ts` `video-tutorial`
...
- **透明**: すべてのスコアと方法論は [juanchi.dev/awesome/sources](https://juanchi.dev/awesome/sources) で公開 (近日)
- **自己調整**: `awesome-*` が維持されなくなると BENCH に落ち、最終的に外れる
- **Human-in-the-loop**: AI だけではない。⭐ GEM は Claude を通過後、[Juan Torchia](https://juanchi.dev) による人間判定
- **再現可能**: この README は週次で再生成。システム解説は [juanchi.dev/blog/series/awesome-curated](https://juanchi.dev/blog/series/awesome-curated)

## 🤝 コントリビューション

- 🐛 ツールの分類ミス? → [Issue を開く](https://github.com/JuanTorchia/awesome-curated/issues/new)
- ➕ 重要なツールが抜けている? → リンク+理由で Issue
- 📝 新しいソースを提案? → スキャンすべき `awesome-*` リポを提案

## 📬 Newsletter

roster に新しく入るツールの月次ダイジェスト → [juanchi.dev/newsletter](https://juanchi.dev)
